...
def chat_prompting_image(question, uploaded_file):

    # Read image
    # Convert to a file-like object openable by PIL
    ...
    # Append the user's question to the conversation history
    conversation_history.append({"role": "user", "content": [{"type": "text", "text": f"{question}, I want {st.session_state['question_number']} total."},
...
conversation_history = [
    ...
    {"role": "user", "content": "If the question involves a table of values, please embed the table directly in the question in HTML format. Only output the python dictionary."},
...
with col1:
    ...
    question = st.text_area("Enter your question prompt:", value = "Create true false questions about the image")
    uploaded_file = st.file_uploader("Choose an image...", type=['jpg', 'jpeg', 'png', "webp","gif"])

    if st.button("Generate"):
        st.session_state['response_text'] = chat_prompting_image(question, uploaded_file)
        #cleaned_response_text = st.session_state['response_text'].replace("```json", "").replace("```", "").strip()
